...
- **저장 헬퍼(같은 파일)**:
  ...
  - `getDiaryPhotosForSave(category, photos, isEditing)`: `photo` 신규 작성은 선택 사진 전체를, 수정은 새로 추가한 사진만 저장 흐름에 전달한다. 다른 카테고리는 `undefined`.
- **사진 입력 계약**: `DiaryPhotoPicker.tsx`는 앨범의 복수 선택과 기기 카메라의 단일 촬영을 각각 제공하며, 두 경로의 결과를 동일한 `DiaryPhotoInput`으로 정규화해 기존 업로드·저장 흐름에 전달한다. 수정 화면에서도 기존 사진과 추가 버튼을 함께 표시한다. 앨범과 카메라 권한은 해당 버튼을 누른 시점에만 요청하며, 같은 반려동물·같은 로컬 날짜의 모든 사진 기록과 새 사진을 합쳐 현재 최대 5장으로 제한한다. 메모와 사진 기록 자체는 하루에 여러 번 저장할 수 있다.
- **향후 플랜 계약(문서 전용)**: 무료 사용자는 하루 5장을 유지하고 결제 사용자는 무제한 사진을 지원할 예정이다. 현재 앱에는 결제 여부에 따른 분기를 넣지 않으며 모든 사용자에게 5장 제한을 적용한다.
- **사진 조회 계약**: 비공개 `pet-media` 객체 경로는 다이어리 조회 시 1시간짜리 서명 URL로 변환한다. 다이어리 목록은 사진 썸네일을 표시하고, 썸네일을 누르면 전체 화면 뷰어를 연다. 원본 `storage_path`를 공개 URL로 직접 노출하지 않는다.
- **네이티브 권한 설정**: `apps/mobile/app.json`의 `expo-image-picker` plugin에 사진·카메라 사용 문구를 명시하고, 사진 촬영만 지원하므로 microphone 권한은 비활성화한다. 권한 설정 변경은 새 native build에서 검증한다.
