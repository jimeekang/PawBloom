...
export function formatReportTimelineItem(item: ReportTimelineItem, language: Language, formatDiarySummary?: DiarySummaryFormatter): string {
  ...
  return `${datePart}${item.time} · ${t("reports.timelineMedication")} ${localizedMedicationName(item.medicationName)}: ${t(doseStatusKeys[item.status ?? "pending"])}${dosage}${given}${reaction}`;
}
