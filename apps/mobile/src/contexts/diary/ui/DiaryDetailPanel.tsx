      ...
      {mealSlots.map((slot) => (
        <View key={slot} style={styles.mealRow}>
          ...
          <View style={styles.mealInputStack}>
            ...
            <TextInput style={styles.compactInput} accessibilityLabel={t("ko", "diary.offeredGrams")} keyboardType="numeric" value={detail.meals[slot]?.offeredGrams ?? ""} onChangeText={(value) => updateMeal(slot, "offeredGrams", value)} placeholder={t("ko", "diary.offeredGrams")} placeholderTextColor={colors.textSoft} />
            <FieldLabel label={t("ko", "diary.eatenGrams")} />
            <TextInput style={styles.compactInput} accessibilityLabel={t("ko", "diary.eatenGrams")} keyboardType="numeric" value={detail.meals[slot]?.eatenGrams ?? ""} onChangeText={(value) => updateMeal(slot, "eatenGrams", value)} placeholder={t("ko", "diary.eatenGrams")} placeholderTextColor={colors.textSoft} />
          </View>
        </View>
      ))}
